define(["jquery","templates"], function($, WebTemplates) {
   ...
   function getSectionLink(id)
   {
      ...
      if (link)
      {
         // 100 times faster than jQuery.extend({}, link)
         return {"url" : link.url, "path" : link.path, "data-link" : link["data-link"]};
      }
      return null;
   };
   ...
   me.getSelectedSection = getSelectedSection;
   ...
   /**
    * Click on the link given by this hash.
    * 
    * @param    hash
    *           id selector, in this form #id.
    */
   function clickNavMenuItem(hash)
   {
      var link = getSectionCanonicalUrl(hash);
      if (link)
      {
         //console.log("clickNavMenuItem('" + hash +"')");
         $('a[href$="' + link + '"]', currentDocument).trigger("click");
      }
   };
   
   me.clickNavMenuItem = clickNavMenuItem;
   /**
   ...
   function hideCurrentSection(newHash)
   {
      ...
      {
         if (selectedSection && newHash != selectedSection)
         {
            var slot = $(selectedSection, currentDocument);
            slot.addClass("hide");
            slot.closest("section").addClass("hide");
            activateNavMenuItem(selectedSection, false);
            selectedSection = newHash;
         }
         else
         {
            ...
            if (visibleFloatSection && newHash != visibleFloatSection)
            {
               ...
               slot.addClass("hide");
               slot.closest("section").addClass("hide");
               activateNavMenuItem(visibleFloatSection, false);
            }
         }
      }
   };
   ...
   function loadSection(anchor, context, dir)
   {
      ...
      if (!url)
      {
         // reject to load an unknown section
         doneNotifier.reject();
         return {doneNotifier : doneNotifier.promise(), load : function() {}};
      }
   ...
   function setup(args)
   {
      ...
      var currentPage = pageHash.current;
      var requestedPage = pageHash.requested;
      
      var anchorNode = $(anchor, context);
      builder.applyTemplates(data, anchorNode, context);
      // execute script on the current page
      // only page's scripts are executed if it is called to build static pages
      if (engineMode && currentPage != "")
      {
      ...
      for (i = pages.length - 1; i >=0; i--) 
      {
         ...
         {
            visibleFloatSection = hash;
            // if currentPage is empty, then it is an active page from web-site.json
            if (currentPage == "")
            {
               currentPage = hash;
            }
         ...
         if (sectionIsDynamic)
         {
            var addNewPage = !engineMode || 
                             (footer || (requestedPage && (hash == requestedPage)) ||
                             (!requestedPage && (hash == visibleFloatSection)));
            if (addNewPage)
            {
      ...
      if (requestedPage)
      {
         ...
         if (hasNotBeenLoaded(requestedPage, parent))
         {
            overrideActivePage = requestedPage;
         }
      }
      //console.log("loadedAtStart = " + JSON.stringify(loadedAtStart));
      var notifiers = loadPagesAtStart(loadedAtStart, context, dir, overrideActivePage, currentPage);
      // fix visible float section
      
      visibleFloatSection = currentPage;
      //console.log("visibleFloatSection = " + visibleFloatSection);
   ...
   function loadPagesAtStart(loadedAtStart, context, dir, overrideActivePage, currentPage)
   {
      ...
      if (overrideActivePage && (overrideActivePage.length > 1) && isFloatSection(overrideActivePage))
      {
         ...
         doneNotifiers.push(loadSection(visibleFloatSection, context, dir));
            ...
            (currentPage == visibleFloatSection))
      {
         doneNotifiers.push(loadSection(visibleFloatSection, context, dir));
      }
